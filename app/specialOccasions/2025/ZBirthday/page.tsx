...
export default function ZBirthday() {
  ...
  useEffect(() => {
    const handle = () => {
      if (heldKey === "w" && top > 0) setTop(top - 0.5);
      if (heldKey === "a" && left > 0) setLeft(left - 0.5);
      if (heldKey === "s" && top < window.innerHeight - characterHeight - 70)
        setTop(top + 0.5);
      if (heldKey === "d" && left < window.innerWidth - 70) setLeft(left + 0.5);
    };
      ...
      {overlayTextIndex < overlayText.length && (
        ...
        >
          ...
          >
            ...
            >
              <p>{overlayText[overlayTextIndex]}</p>
